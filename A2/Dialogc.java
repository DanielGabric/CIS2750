...
public class Dialogc extends JFrame implements KeyListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (a.equals("ApproveSelection")) {
                ...
                if(isCompile&&!isNewPage()){
                    /*Add title.length*/compile.compile(filePath , title,workingDirectory);
                }
                if(isRun&&!isNewPage()){
                    boolean isGood=compile.compileThenRun(filePath,workingDirectory,title,runComp,argsComp,true);
                    if(isGood)
                        compile.compileThenRun(filePath,workingDirectory,title,runJVM,argsJVM,false);
                    else
    ...
    private class CompileAndRunListener implements ActionListener {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if(!hasBeenChanged&&!isNewPage()){
                Compiler compile = new Compiler();
                boolean canBeRan = compile.compileThenRun(filePath,workingDirectory,title,runComp,argsComp,true);
                if(canBeRan)
                    compile.compileThenRun(filePath,workingDirectory,title,runJVM,argsJVM,false);
                else
                    JOptionPane.showMessageDialog(null,"Error! Action Listeners for buttons have not been implemented yet!");
    ...
    public void setUpMenuBar() {
        ...
        JMenuItem[] itemConfig = {new JMenuItem("Java Compiler"), new JMenuItem("Compile options"),
            new JMenuItem("Java Run-time"), new JMenuItem("Run-time options"), new JMenuItem("Working Directory")};
