...
A4 - Notes:
- when giving a path to a directory, you need path from the root directory, so from '~'
- FOR DATABASES - if the database already exists the program will delete it and recreate it <--- READDD THISSSSS PLEASE
- MAXIMUM of 25 characters in any string field for DATABASES and also only a float with 4 digits before decimal point and 2 after
- When putting in directories in install script, it is full path from root
